...
#pragma warning 549        // 屏蔽警告
    ...
    GaussianFilter(Image_Data);
    const int crop_start_row = (int)(LCDH * 0.75); // 计算裁剪起始行
    for (int i = crop_start_row; i < LCDH; i++) {
        for (int j = 0; j < LCDW; j++) {
            Image_Use[i][j] = 0; // 将底部25%区域置零
        }
    }
//    for (int y = 1; y < LCDH-1; y++) {
//        for (int x = 1; x < LCDW-1; x++) {
//            unsigned int sum = 0;
//            sum += Image_Use[y-1][x-1] * 1;
//            sum += Image_Use[y-1][x]   * 2;
//            sum += Image_Use[y-1][x+1] * 1;
//            sum += Image_Use[y][x-1]   * 2;
//            sum += Image_Use[y][x]     * 4; // 中心权重最高
//            sum += Image_Use[y][x+1]   * 2;
//            sum += Image_Use[y+1][x-1] * 1;
//            sum += Image_Use[y+1][x]   * 2;
//            sum += Image_Use[y+1][x+1] * 1;
//            Image_Use[y][x] = sum / 16; // 归一化
//        }
//    }
#endif
...
void Get_Bin_Image(unsigned char mode)
{
    ...
    if (mode == 0)
    {
        Threshold = GetOSTU_Stable(Image_Use); // 大津法阈值
        Threshold+=30;//调整这个数据，如果有干扰，调整这个数据，调到仅有白线
        if((uint16)(Threshold-last_Threshold)<5)
    ...
    if (mode == 4) { // 新增自适应阈值模式
        AntiGlare_Binarization(Image_Use, Pixle, 15, 10);
        for(int i=1; i<LCDH-1; i++) {
             for(int j=1; j<LCDW-1; j++) {
                 ...
                 if(Pixle[i][j] == 0) {
                     ...
                     for(int m=-1; m<=1; m++) {
                         for(int n=-1; n<=1; n++) {
                             if(Pixle[i+m][j+n] == 0) count++;
                         }
                     }
                     if(count < 3) Pixle[i][j] = 255;
                 }
             }
         }
     }
...
//                    if (Pixle[i+dy][j+dx] == 255) count++;
//                }
//            }
//            // 若周围少于2个白点，视为噪声
//            if (count < 2) Pixle[i][j] = 0;
//        }
//    }
}
...
int GetOSTU(unsigned char tmImage[LCDH][LCDW])
{
    ...
    signed short Threshold = 0;
    unsigned char HistoGram[256];
    int process_rows = (LCDH * 3) / 4; // 计算处理的行数
